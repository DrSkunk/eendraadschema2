...
export class SituationPlan {
  ...
  addElementFromFile(
    ...
    element.setVars({
      ...
      posx: posx,
      posy: posy,
      labelfontsize: this.defaults.fontsize,
  ...
  addElementFromElectroItem(
    ...
    const electroItem: Electro_Item =
      globalThis.structure.getElectroItemById(electroItemId);
  ...
  syncToEendraadSchema() {
    for (let element of this.elements) {
      ...
      if (
        (element as any).electroItemId != null &&
        globalThis.structure.getElectroItemById(element.getElectroItemId()) ==
          null
      ) {
  ...
  orderByZIndex() {
    ...
    this.elements.sort((a, b) => {
      let asort =
        a.boxref == null || a.boxref.style.zIndex === ""
          ? 0
          : parseInt(a.boxref.style.zIndex);
      let bsort =
        b.boxref == null || b.boxref.style.zIndex === ""
          ? 0
          : parseInt(b.boxref.style.zIndex);
      return asort - bsort;
    });
  }
  ...
  countByElectroItemId(electroItemId: number): number {
    return this.elements.filter(
      (element) => element.getElectroItemId() === electroItemId
    ).length;
  }
  ...
  toSitPlanPrint(fitToPage: boolean = false): any {
    ...
    let outstruct: any = {};
    outstruct.numpages =
      this.elements.length > 0 ? globalThis.structure.sitplan.numPages : 0;
    outstruct.pages = [];
    ...
    for (let i = 0; i < outstruct.numpages; i++) {
      ...
      for (let element of this.elements) {
        if (element.page == i + 1) {
          let fontsize =
            element.labelfontsize != null ? element.labelfontsize : 11;
          svgstr += element.getScaledSVG(true);

          if (fitToPage) {
            let boundingbox = getRotatedRectangleSize(
              element.sizex * element.getscale(),
              element.sizey * element.getscale(),
              element.rotate
            );
      ...
      svgstr = `<svg xmlns="http://www.w3.org/2000/svg" width="${
        maxx - minx
      }px" height="${maxy - miny}px" viewBox="${minx} ${miny} ${maxx - minx} ${
        maxy - miny
      }">${svgstr}</svg>`;

      outstruct.pages.push({
        sizex: maxx - minx,
        sizey: maxy - miny,
        svg: svgstr,
      });
