...
export class SituationPlanElement {
  ...
  isEendraadschemaSymbool(): boolean {
    if (this.electroItemId != null) {
      return (
        globalThis.structure.getElectroItemById(this.electroItemId) != null
      );
    }
    return false;
  }
  ...
  private static readonly ROTATES_360_DEGREES_TYPES = new Set([
    "Contactdoos",
    "Lichtpunt",
    "Drukknop",
    "Media",
  ...
  isEDSSymbolAndRotates360degrees(): boolean {
    if (this.isEendraadschemaSymbool()) {
      let electroElement: Electro_Item =
        globalThis.structure.getElectroItemById(this.electroItemId);
  ...
  updateElectroItemSVG(
    ...
    // Set the SVG string and update the size, this only works with an electro item
    if (this.isEendraadschemaSymbool()) {
      if (this.svg !== svg) {
        // This works because when saving to a file, svg is set to '' so an update will be triggered here
        this.needsViewUpdate = true;
        this.svg = svg;
  ...
  public berekenAfbeeldingsRotatieEnSpiegeling = (): [number, boolean] => {
    ...
    if (globalThis.structure.properties.legacySchakelaars == false) {
      if (this.isEendraadschemaSymbool()) {
        let electroItem = globalThis.structure.getElectroItemById(
          this.electroItemId
        );
        if (electroItem != null) {
          if (
            electroItem.props.type == "Schakelaars" &&
            (electroItem.props.aantal_schakelaars == 1 ||
              electroItem.props.aantal_schakelaars == null)
          ) {
            if (
              [
                "enkelpolig",
                ...
                "kruis_enkel",
                "dimschakelaar",
                "dimschakelaar wissel",
                "rolluikschakelaar",
              ].includes(electroItem.props.type_schakelaar)
            )
  ...
  getScaledSVG(positioned: boolean = false): string {
    ...
    if (positioned) {
      // Indien we de SVG willen positioneren en roteren, bvb voor gebruik in een print

      posinfo = `x="${this.posx - (this.sizex / 2) * this.scale}" y="${
        this.posy - (this.sizey / 2) * this.scale
      }"`;

      const [rotate, spiegel] = this.berekenAfbeeldingsRotatieEnSpiegeling();

      transform = `transform="rotate(${rotate} ${this.posx} ${this.posy})${
        spiegel ? ` scale(-1,1) translate(${-this.posx * 2} 0)` : ""
      }"`;
      ...
      return `<svg class="svg-icon" width="${
        this.sizex * this.scale
      }px" height="${this.sizey * this.scale}px" viewBox="0 0 ${this.sizex} ${
        this.sizey
      }">${coloredSvg}</svg>`;
   ...
   */
  scaleSelectedBoxToPaperIfNeeded(
    maxx: number,
    maxy: number,
  ...
  fromJsonObject(json: any) {
    ...
    this.adreslocation =
      json.adreslocation != null ? json.adreslocation : "rechts";

    this.rotate = json.rotate != null ? json.rotate : 0;
    this.scale =
      json.scale != null ? json.scale : globalThis.SITPLANVIEW_DEFAULT_SCALE;
