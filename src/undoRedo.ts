...
class jsonStore {
  ...
  undoStackSize(): number {
    return Math.max(this.undoStack.length - 1, 0);
  }
  redoStackSize(): number {
    return Math.max(this.redoStack.length, 0);
  }
}
...
export class undoRedo {
      ...
      for (let element of globalThis.structure.sitplan.getElements()) {
        if (!element.isEendraadschemaSymbool())
          element.svg = this.largeStrings
            .pushIfNotExists(element.getUnscaledSVGifNotElectroItem())
            .toString();
      }
    }
  }
  ...
  getOptions(): string {
    ...
    if (globalThis.structure.sitplanview != null) {
      options.selectedBoxesOrdinals =
        globalThis.structure.sitplanview.getSelectedBoxesOrdinals();
      if (
        (globalThis.structure.sitplanview.sideBar as any).getUndoRedoOptions !=
        null
      ) {
        Object.assign(
          options,
  ...
  reload(text: string | null, options: any) {
    ...
    switch (globalThis.currentReactView) {
      case "sitplan":
        ...
        if (
          (globalThis.structure.sitplanview.sideBar as any)
            .setUndoRedoOptions != null
        )
          (globalThis.structure.sitplanview.sideBar as any).setUndoRedoOptions(
            options
          );
        ...
        for (let selectedBox of options.selectedBoxesOrdinals) {
          if (globalThis.structure.sitplan.getElements().length <= selectedBox)
            break;
  ...
  undoStackSize(): number {
    return this.historyEds.undoStackSize();
  }
  redoStackSize(): number {
    return this.historyEds.redoStackSize();
  }
}
