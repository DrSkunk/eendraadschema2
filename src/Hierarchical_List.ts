...
export class Hierarchical_List {
  ...
  reSort() {
    // niet de meest elegante implementate maar draait in 2 ms op een lang schema dus meer dan goed genoeg

    let isSorted = () => {
      // Controleer of de data reeds gesorteerd is. Controleert niet of Id's dubbel voorkomen, dat laatste mag normaal niet gebeuren
      ...
      for (let i: number = 0; i < this.length; i++) {
        if (this.active[i]) {
          ...
          if (parentId == stacktop) {
            // We zakken 1 level dieper
            stack.push(stacktop);
            stacktop = this.id[i];
          } else {
            // We blijven op hetzelfde niveau of stijgen terug op tot we onder de parent hangen
  ...
  createContainerIfNotExists(): Electro_Item {
    ...
    for (let i = 0; i < this.length; i++) {
      if (
        ...
        this.data[i] instanceof Container
      ) {
  ...
  createItem(electroType: string): Electro_Item {
    ...
    switch (electroType) {
      ...
      case "Leeg":
        ...
        break;
      case "Aardingsonderbreker":
        tempval = new Aardingsonderbreker(this);
        break;
      case "Aftakdoos":
  ...
  moveUp(my_id: number) {
    ...
    for (let i = currentOrdinal - 1; i >= 0; i--) {
      if (
        this.data[i].parent == currentparent &&
        this.active[i] &&
        !(this.data[i] as Electro_Item).isAttribuut()
      ) {
        newOrdinal = i;
  ...
  clone(my_id: number, parent_id?: number) {
    ...
    let my_item = this.createItem(
      (this.data[currentOrdinal] as Electro_Item).getType()
    );
    my_item.clone(this.data[currentOrdinal]);

    // Now add the clone to the structure. The clone will have id this.curid-1
    if (arguments.length < 2) this.insertItemAfterId(my_item, my_id);
    //Cloning the top-element, this messes up the ordinals !!
    ...
    for (let i = 0; i < this.length; i++) {
      if (this.id[i] == my_id) {
        for (let j = this.length - 1; j >= 0; j--) {
          //We need to loop in opposite sense
    ...
    if (parent != null) {
      if (parent.getType() == "Kring") {
        ...
        if (item.getType() == "Omvormer" && item.props.inkring) {
          ...
          if (myOrdinal < lastOrdinalInKring) {
            // Teken een verticale lijn over de volledige hoogte
            y1 = 0;
            y2 = mySVG.yup + mySVG.ydown;
          } else {
            // Teken een verticale lijn over de halve hoogte
            y1 = mySVG.yup;
            y2 = mySVG.yup + mySVG.ydown;
          }
      ...
      if (
        parent.getType() == "Kring" ||
        ["Domotica module (verticaal)"].includes(parent.getType())
      ) {
        ...
        mySVG.data +=
          ...
          htmlspecialchars(displaynr) +
          "</text>";
      }
    }
  }

  // Legacy DOM methods removed - React handles all UI updates now
  ...
  voegAttributenToeAlsNodigEnReSort() {
        ...
        if (parentItem.props.heeft_externe_sturing == true) {
          switch (parentItem.props.type_externe_sturing) {
            case "schakelaar":
              ...
              this.data[i].resetProps();
              this.data[i].props.isAttribuut = true;
              continue;
            case "drukknop":
    ...
    for (let i = 0; i < this.length; i++) {
      ...
      if (
        this.active[i] &&
        (this.data[i] as Electro_Item).getType() ==
  ...
  toHTML(myParent: number) {
    // Als we alles tekenen mogen we renumber niet vergeten
    if (myParent == 0) {
      this.reNumber();
    }
    ...
    for (let i = 0; i < this.length; i++) {
      if (
        ...
        if (electroItemType != null && electroItemType == "Kring")
          aantalKringen++;

        // Generate HTML for this item (React mode - no longer using toHTMLinner)
        output +=
          '<table class="html_edit_table" id="id_elem_' + this.id[i] + '">';
        if (this.data[i].collapsed) {
          output += `<tr>
                        <td bgcolor="#8AB2E4" onclick="HLCollapseExpand(${
                          this.data[i].id
                        })" valign="top" title="Klik om uit te vouwen">▶</td>
                        <td width="100%">${this.data[i].toHTML(
                          this.mode
                        )}<br></td>
                    </tr>`;
        } else {
          output += `<tr>
                       <td bgcolor="C0C0C0" onclick="HLCollapseExpand(${
                         this.data[i].id
                       })" valign="top" title="Klik om in te vouwen">▼</td>
                       <td width="100%">${this.data[i].toHTML(
                         this.mode
                       )}<br>${this.toHTML(this.id[i])}</td>
                    </tr>`;
        }
        output += "</table>";
      }
    ...
    if (myParent == 0 && aantalElementen < 1) {
      output +=
        '<button onclick="HLAdd()">Voeg eerste object toe of kies bovenaan "Nieuw"</button><br>'; //no need for the add button if we have items
    }
    ...
    if (parent != null && parent.getType() == "Kring" && aantalKringen > 1) {
      const EDStekenFoutKleur = trimString(
  ...
  findKringName(my_id: number): string {
    ...
    if (myParent == 0) {
      ...
      if ((this.data[myParentOrdinal] as Electro_Item).getType() == "Kring") {
        let kringnaam: string = trimString(
          this.data[myParentOrdinal].props.naam
        );
        if (kringnaam != "") {
  ...
  updateKringNamen() {
    function changeInLeftColIfOpen(id: number, newname: string) {
      let div: HTMLInputElement = document.getElementById(
        `HL_edit_${id}_naam`
      ) as HTMLInputElement;
      if (div) {
    ...
    function vindVolgendeKringNaam(kringNamen: {
      [kring: string]: boolean;
    }): string {
    ...
    function bestaandeKringNamen(): { [kring: string]: boolean } {
      ...
      for (let i = 0; i < this.length; i++) {
        ...
        if (
          electroType == "Kring" &&
          electroItem.props.autoKringNaam !== "auto"
        ) {
    ...
    let kringNamen: { [kring: string]: boolean } =
      bestaandeKringNamen.call(this);
  ...
  reNumber() {
    ...
    for (let i = 0; i < this.length; i++) {
      ...
      if (electroType == "Bord") {
        ...
        !contains(itemsZonderNr, (this.data[i] as Electro_Item).getType())
      ) {
        let electroItem: Electro_Item = this.data[i] as Electro_Item;
        if (electroItem == null) continue;
        let parent: Electro_Item = electroItem.getParent() as Electro_Item;
        if (parent == null) continue;
        if (
          parent.getType() == "Kring" ||
          parent.getType() == "Domotica module (verticaal)"
        ) {
          if (electroItem.props.autonr === "auto") {
            ...
            if (
              electroItem.props.nr != null &&
              !isNaN(Number(electroItem.props.nr))
            )
              lastNumbers[kringnaam] = Number(electroItem.props.nr);
          }
        }
      }
    }
  ...
  toSVG(
    ...
    //stack can be "horizontal" or "vertical"
    ...
    let inSVGIds: Array<number> = new Array<number>(); // Track element IDs for interactive SVG
    ...
    for (let i = 0; i < this.length; i++) {
      if (
        ...
        !(this.data[i] as Electro_Item).isAttribuut() &&
        (this.data[i].parent == myParent ||
          (includeparent == true && this.id[i] == myParent))
      ) {
        let mytype: string = (this.data[i] as Electro_Item).getType();
        /*if (typeof(this.data[i].keys) != 'undefined') mytype = this.data[i].keys[0][2]; else*/ mytype =
          this.data[i].props.type;
        switch (mytype) {
          case "":
          ...
          case "Domotica":
            ...
            this.tekenVerticaleLijnIndienKindVanKring(
              this.data[i] as Electro_Item,
              inSVG[elementCounter]
            );
            break;
          ...
          default:
            const parentElectroItem = this.getElectroItemById(
              myParent
            ) as Electro_Item | null;
            if (
              myParent != 0 &&
              ...
              inSVG[elementCounter] = this.toSVG(
                this.id[i],
                "horizontal",
                0,
                true
              ); //if we are still in vertical mode, switch to horizontal and take childs with us
            } else {
              //we are in horizontal mode and can start drawing
              if (this.id[i] == myParent) {
                // Element is de parent en tekent zichzelf
                inSVG[elementCounter] = this.data[i].toSVG();
              } else {
                // Element is niet de parent, we tekenen het element en al zijn kinderen
                inSVG[elementCounter] = this.toSVG(
                  this.id[i],
                  "horizontal",
                  0,
        ...
        if (mytype !== "Container") {
          inSVGIds[elementCounter] = this.id[i]; // Track element ID for interactive SVG
          elementCounter++;
        }
      }
    }
    ...
    switch (stack) {
      ...
      case "vertical":
        ...
        if (parentItem != null && parentItem.getType() == "Kring")
          max_xright += 10; // Altijd 10 extra rechts voorzien voor uitstekende tekst/adressen

        if (
          !(this.getElectroItemById(myParent) instanceof Omvormer) &&
    ...
    if (myParent == 0) {
      //We will always foresee a 20 pixel horizontal and 5 pixel vertical margin
      let header: string =
        '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" transform="scale(1,1)" width="' +
        (width + 20) +
        '" height="' +
        (height + 5) +
        '">';
